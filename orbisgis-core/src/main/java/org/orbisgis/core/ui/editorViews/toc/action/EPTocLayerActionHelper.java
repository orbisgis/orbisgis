...
public class EPTocLayerActionHelper extends
	...
	public static void execute(MapContext mapContext, ILayerAction action,
		...
		if (selectedResources.length == 0) {
			action.execute(mapContext, null);
		} else {
			for (ILayer resource : selectedResources) {
				action.execute(mapContext, resource);
			}
		}
	}

	public static void execute(MapContext mapContext,
			IMultipleLayerAction layerAction, ILayer[] layerArray) {
		layerAction.executeAll(mapContext, layerArray);
	}
