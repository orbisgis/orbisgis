		...
		for (int i = 0; i < editors.length; i++) {
			if (editors[i].acceptElement(element.getTypeId())) {
					...
					editor = editors[i];
                                ...
                                if (editor instanceof MapEditorPlugIn) {
                                        ed = new EditorDecorator(editor, IconLoader.getIcon("map.png"), Names.EDITOR_MAP_ID);
                                } else if (editor instanceof TableEditorPlugIn) {
                                        ed = new EditorDecorator(editor, IconLoader.getIcon("openattributes.png"), Names.EDITOR_TABLE_ID);
                                }
                        }
                }
                return ed;
