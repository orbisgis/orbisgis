...
public class SourceListModel extends AbstractListModel implements ListModel {
	...
	private IGeocatalogFilter[] filters = new IGeocatalogFilter[0];
	...
	private void refresh() {
		...
		if (nameFilter != null) {
			IGeocatalogFilter textFilter = new TextFilter();
	...
	private String[] filter(SourceManager sourceManager, String[] names,
			IGeocatalogFilter filter) {
		ArrayList<String> filteredNames = new ArrayList<String>();
		for (String name : names) {
			if (filter.accept(sourceManager, name)) {
	...
	public void filter(String text, IGeocatalogFilter[] filters) {
	...
	private final class OrFilter implements IGeocatalogFilter {

		@Override
		public boolean accept(SourceManager sm, String sourceName) {
			for (int i = 0; i < filters.length; i++) {
				if (filters[i].accept(sm, sourceName)) {
					return true;
				}
			}
			return false;
	...
	private final class TextFilter implements IGeocatalogFilter {
