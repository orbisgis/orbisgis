...
public abstract class AbstractLineTool extends Line implements InsertionTool {
	...
	public void transitionTo_Done(MapContext vc, ToolManager tm)
		...
		if (!g.isValid()) {
			throw new TransitionException(I18N.getText("org.orbisgis.core.ui.editors.map.tool.LineTool.0")); //$NON-NLS-1$
		}
		lineDone(ls, vc, tm);
	...
	public void drawIn_Point(Graphics g, MapContext vc, ToolManager tm)
		...
		if (!ls.isValid()) {
			throw new DrawingException(I18N.getText("org.orbisgis.core.ui.editors.map.tool.LineTool.0")); //$NON-NLS-1$
		}
	}
