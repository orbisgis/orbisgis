        ...
        public ILayer createLayer(DataSource sds) throws LayerException {
                ...
                if ((type & SourceManager.WMS) == SourceManager.WMS) {
                        ...
                        if ((type & SourceManager.VECTORIAL) == 0 && 
								...
								.error(I18N.getString("Cannot close the data source"), e);
                                        }
                                        hasSpatialData = (sfi != -1);
                                } catch (DriverException e) {
                                        throw new LayerException(I18N.getString("Cannot check source contents"), e);
                                }
                        }
                        if (hasSpatialData) {
                                return new Layer(sds.getName(), sds);
                        } else {
                                throw new LayerException(I18N.getString("The source contains no spatial info"));
                        }
                }
        }
