...
public class IntegerUnionTest {
        ...
        private void check(IntegerUnion union, Integer[] check) {
                List<Integer> ints = union.getRowRanges();
                assertTrue("Union fail, check arrays had differents size",
                        ints.size()==check.length);
                int i=0;
                for(Integer value : union) {
                        if(value!=check[i]) {
                                assertTrue("Union fails, different values "+value+"!="+check[i],value==check[i]);
                        }
                        i++;                        
                }
        } 
        ...
        Iterator<Integer> it=mergeTool.getRowRanges().iterator();
