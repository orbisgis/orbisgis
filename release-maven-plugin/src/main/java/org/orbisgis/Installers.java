 ...
 */
public class Installers extends AbstractMojo {
		...
		if (lzPackHome == null) {
			...
			try {
				ExecutionUtils.executeThread(new String[] {
						...
						".", "-o",
						"orbisgis-linux-installer-" + version + ".jar", "-h",
						lzPackHome }, null);
				ExecutionUtils.executeThread(new String[] {
						lzPackHome + "/bin/compile", "install-windows.xml", "-b",
						".", "-o",
						"orbisgis-windows-installer-" + version + ".jar", "-h",
						lzPackHome }, null);
			} catch (IOException e) {
