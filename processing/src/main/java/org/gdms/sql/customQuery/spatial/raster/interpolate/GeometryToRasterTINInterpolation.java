...
import org.gdms.algorithm.triangulation.TINToRaster;
...
/**
 * This function is used to build a raster using a set of points.
 * We assume that the geometry contains the z value.
 * 
...
public class GeometryToRasterTINInterpolation implements CustomQuery {
	...
	public ObjectDriver evaluate(DataSourceFactory dsf, DataSource[] tables,
		...
		try {
			...
			TINToRaster tinToRaster = new TINToRaster(values[1].getAsDouble(), numberOfPoints, xVcl, yVcl, zVcl);

			GeoRaster georaster = tinToRaster.getGeoRaster();
			
			/*
			 * To understand the bug uncomment the code
			 * try {
				georaster.show();
			} catch (IOException e) {
			...
			*/
			...
			driver.addValues(new Value[] {
					ValueFactory.createValue(georaster)});
	...
	public String getSqlOrder() {
		return "select Interpolate() from mydata;";
	}
}