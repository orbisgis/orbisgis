...
/*
 select register('/tmp/1.triangle', 'r');
 create table r as select * from points;
 */
...
//				.isOnlyComposedOfPointsOrMultiPoints(sds);
//
//		if (isOfNodeFileType) {
//			final String nodeFileName = removeFilenameExtension(file) + ".node";
//			final NodeWriter nodeWriter = new NodeWriter(
//					new File(nodeFileName), sds);
//			nodeWriter.write();
//			nodeWriter.close();
//		} else {
//			System.err.println("_____________============------------------");
//
//			final String polyFileName = removeFilenameExtension(file) + ".poly";
//			final PolyWriter polyWriter = new PolyWriter(
//					new File(polyFileName), sds);
//			polyWriter.write();
//			polyWriter.close();
//		}
	}
	...
	private String removeFilenameExtension(File file) {
		...
		return tmp.substring(0, tmp.length() - 9);
	}
