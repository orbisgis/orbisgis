...
public class JPanelLegendList extends javax.swing.JPanel implements UIPanel {
	...
	private void initComponents() {
		...
		jPanel2Layout.setVerticalGroup(jPanel2Layout.createParallelGroup(
				javax.swing.GroupLayout.Alignment.LEADING).addGap(0, 256,
				Short.MAX_VALUE));
	...
	private void jButtonMenuAddActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jButtonMenuAddActionPerformed
		...
		if ((constraint == null)
				|| ((constraint != GeometryConstraint.LINESTRING) && (constraint != GeometryConstraint.MULTI_LINESTRING))) {
	...
	private void jList1ValueChanged(javax.swing.event.ListSelectionEvent evt) {// GEN-FIRST:event_jList1ValueChanged
		...
		if (dec != null) {
			...
			switch (legType) {
			case UNIQUESYMBOL:
				pan = LegendPanelFactory.createPanel(
						LegendPanelFactory.UNIQUE_SYMBOL_LEGEND,
						(UniqueSymbolLegend) dec.getLegend(), constraint, null,
						true);
				break;
			case UNIQUEVALUE:
				pan = LegendPanelFactory.createPanel(
						LegendPanelFactory.UNIQUE_VALUE_LEGEND,
						(UniqueValueLegend) dec.getLegend(), constraint, layer,
						false);
				break;
			case INTERVAL:
				pan = LegendPanelFactory.createPanel(
						LegendPanelFactory.INTERVAL_LEGEND,
						(IntervalLegend) dec.getLegend(), constraint, layer,
						false);
				break;
			case PROPORTIONAL:
				pan = LegendPanelFactory.createPanel(
						LegendPanelFactory.PROPORTIONAL_LEGEND,
						(ProportionalLegend) dec.getLegend(), constraint,
						layer, false);
				break;
			case LABEL:
				pan = LegendPanelFactory.createPanel(
						LegendPanelFactory.LABEL_LEGEND, (LabelLegend) dec
	...
	public Legend[] getLegend() {
		...
		for (int i = 0; i < image.length; i++) {
			try {
				ImageIO.write(image[i], "png", new File("/tmp/"
						+ legendsL[i].getName() + "__" + i + ".png"));
			} catch (IOException e) {
