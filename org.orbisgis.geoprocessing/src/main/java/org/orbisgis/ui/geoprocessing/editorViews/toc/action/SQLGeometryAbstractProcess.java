 ...
 */
package org.orbisgis.ui.geoprocessing.editorViews.toc.action;
...
public abstract class SQLGeometryAbstractProcess implements ILayerAction {
	public void execute(MapContext mapContext, ILayer layer) {
		try {

			final String result = evaluateResult(layer, mapContext);

			if (null != result) {
				final DataSourceFactory dsf = ((DataManager) Services
						.getService(DataManager.class)).getDSF();

				DataSource dsResult = dsf.getDataSourceFromSQL(result);

				// populate the GeoView TOC with a new datasource
			...
			e.printStackTrace();
		} catch (org.orbisgis.core.layerModel.LayerException e) {
			e.printStackTrace();
		}
	}
