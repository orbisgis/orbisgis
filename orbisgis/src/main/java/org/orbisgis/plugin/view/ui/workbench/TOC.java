		...
		try {
			...
			if (t.isDataFlavorSupported(DataFlavor.stringFlavor)) {
				...
				String name = (String) t
						.getTransferData(DataFlavor.stringFlavor);// Name
				// contains
				// the name
				// of the
				// Datasource
	...
	private class MyMouseAdapter extends MouseAdapter {
		...
		private void ShowPopup(MouseEvent e) {
			if (e != null && setTreePath(e) && e.isPopupTrigger()) {
				...
				System.out.println("Popup sur "
						+ TOC.this.currentLayer.getName());
			}
		}

		/**
		 * setTreePath allows to update the treePath and the currentLayer
		 * variables it should be called each time you need parameters of the
		 * current selection
		 * 
		 * @param e
		...
		private boolean setTreePath(MouseEvent e) {
			...
			if (treePath != null) {
				TOC.this.currentLayer = (ILayer) treePath
	...
	private class ActionsListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if ("ADDSLD".equals(e.getActionCommand())) {
				OurFileChooser ofc = new OurFileChooser("sld",
						"SLD file (*.sld)", false);
