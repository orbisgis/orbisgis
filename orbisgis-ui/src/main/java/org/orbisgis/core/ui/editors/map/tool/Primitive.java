...
public class Primitive {
        ...
        private Coordinate[] insertVertexInLinearRing(Coordinate[] coords,
                ...
                if (index != -1) {
                        ...
                        ret[index + 1] = new Coordinate(vertexPoint.getX(), vertexPoint.getY());
        ...
        private Geometry insertVertexInPolygon(Geometry geometry,
                ...
                Coordinate[] inserted = insertVertexInLinearRing(p.getExteriorRing().getCoordinates(), vertexPoint, tolerance);
                if (inserted != null) {
                        LinearRing[] holes = new LinearRing[p.getNumInteriorRing()];
                        for (int i = 0; i < holes.length; i++) {
                                holes[i] = gf.createLinearRing(p.getInteriorRingN(i).getCoordinates());
                        }
                        Polygon ret = gf.createPolygon(gf.createLinearRing(inserted), holes);
                ...
                for (int i = 0; i < p.getNumInteriorRing(); i++) {
                        inserted = insertVertexInLinearRing(p.getInteriorRingN(i).getCoordinates(), vertexPoint, tolerance);
                        if (inserted != null) {
                                LinearRing[] holes = new LinearRing[p.getNumInteriorRing()];
                                for (int h = 0; h < holes.length; h++) {
                                        if (h == i) {
                                                ...
                                                holes[h] = gf.createLinearRing(p.getInteriorRingN(h).getCoordinates());
                                        }
                                }

                                Polygon ret = gf.createPolygon(gf.createLinearRing(p.getExteriorRing().getCoordinates()), holes);
