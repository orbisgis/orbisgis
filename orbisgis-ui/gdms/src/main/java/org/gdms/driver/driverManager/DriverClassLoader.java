...
public class DriverClassLoader extends URLClassLoader {
        ...
        private HashMap<String, ZipFile> clasesJar = new HashMap<String, ZipFile>();
        private String baseDir;
        private static final Logger LOG = Logger.getLogger(DriverClassLoader.class);
        ...
        protected Class<? extends Object> loadClass(String name, boolean resolve)
                ...
                try {
                        ...
                        if (jar == null) {
                                ArrayList<DriverClassLoader> cls = DriverClassLoaderManager.getClassLoaderList(name);

                                if (cls != null) {
                                        for (int i = 0; i < cls.size(); i++) {
                                                c = (cls.get(i)).loadClass(name,
                                                        resolve);
        ...
        public Class<? extends Driver>[] getDrivers() throws ClassNotFoundException {
                LOG.trace("Getting all drivers");
                ArrayList<Class<?>> drivers = new ArrayList<Class<?>>();
                Iterator<String> e = clasesJar.keySet().iterator();
                ...
                return (Class<? extends Driver>[]) drivers.toArray(new Class[drivers.size()]);
        }
}
