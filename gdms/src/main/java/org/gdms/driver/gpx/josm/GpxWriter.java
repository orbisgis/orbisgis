...
public class GpxWriter extends XmlWriter {
	...
	public GpxWriter() {
		super(null);
		// sorry for this one here, this will be cleaned up once the new scheme
		// works
	}
	...
	public void write(GpxData data) {
		...
		out
				.println("<gpx version=\"1.1\" creator=\"JOSM GPX export\" xmlns=\"http://www.topografix.com/GPX/1/1\"\n"
	...
	private void writeAttr(Map<String, Object> attr) {
		...
		// for now it works, but future extension could get very complex and
		// unmaintainable
	...
	private void writeMetaData() {
		...
		if (attr.containsKey(GpxData.META_AUTHOR_NAME)
			...
			if (attr.containsKey(GpxData.META_AUTHOR_EMAIL)) {
				String[] tmp = ((String) attr.get(GpxData.META_AUTHOR_EMAIL))
						.split("@");
				if (tmp.length == 2) {
		...
		if (attr.containsKey(GpxData.META_COPYRIGHT_LICENSE)
			...
			openAtt("copyright", "author=\""
					+ attr.get(GpxData.META_COPYRIGHT_AUTHOR) + "\"");
			if (attr.containsKey(GpxData.META_COPYRIGHT_YEAR)) {
				simpleTag("year", (String) attr
						.get(GpxData.META_COPYRIGHT_YEAR));
			}
			if (attr.containsKey(GpxData.META_COPYRIGHT_LICENSE)) {
				simpleTag("license", encode((String) attr
						.get(GpxData.META_COPYRIGHT_LICENSE)));
			}
			closeln("copyright");
		}
		...
		if (attr.containsKey(GpxData.META_LINKS)) {
			for (GpxLink link : (Collection<GpxLink>) attr
		...
		Envelope bounds = data.recalculateBounds();
		if (bounds != null) {
			String b = "minlat=\"" + bounds.getMinX() + "\" minlon=\""
					+ bounds.getMinY() + "\" maxlat=\"" + bounds.getMaxX()
					+ "\" maxlon=\"" + bounds.getMaxY() + "\"";
			inline("bounds", b);
		}
	...
	/**
	 * if content not null, open tag, write encoded content, and close tag else
	 * do nothing.
	 */
	...
	private void wayPoint(WayPoint pnt, int mode) {
		...
		case TRACK_POINT:
			...
			break;
		default:
			throw new RuntimeException("Unknown mode {0}.");
		}
		if (pnt != null) {
			Coordinate c = pnt.getCoor();
			// openAtt(type, "lat=\"" + c.lat() + "\" lon=\"" + c.lon() + "\"");
			openAtt(type, "lat=\"" + c.x + "\" lon=\"" + c.y + "\"");
