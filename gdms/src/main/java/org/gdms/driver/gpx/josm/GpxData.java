 ...
 * It uses GPX v1.1, see
 * {@link <a href="http://www.topografix.com/GPX/1/1/">the spec</a>} for
 * details.
 * 
...
public class GpxData extends WithAttributes {
	...
	public static final String META_COPYRIGHT_AUTHOR = META_PREFIX
			+ "copyright.author";
	public static final String META_COPYRIGHT_LICENSE = META_PREFIX
			+ "copyright.license";
	public static final String META_COPYRIGHT_YEAR = META_PREFIX
			+ "copyright.year";
	...
	public void mergeFrom(GpxData other) {
		...
		for (Map.Entry<String, Object> ent : other.attr.entrySet()) {
			...
			if (k.equals(META_LINKS) && attr.containsKey(META_LINKS)) {
				((Collection<GpxLink>) attr.get(META_LINKS))
						.addAll((Collection<GpxLink>) ent.getValue());
			} else {
	...
	public Envelope recalculateBounds() {
		Envelope bounds = null;
		for (WayPoint wpt : waypoints) {
			if (bounds == null) {
				bounds = new Envelope(wpt.getCoor());
			} else {
				bounds.expandToInclude(wpt.getCoor());
			}
		}
		for (GpxRoute rte : routes) {
		...
		for (GpxTrack trk : tracks) {
			Envelope trkBounds = trk.getBounds();
			if (trkBounds != null) {
				if (bounds == null) {
					bounds = new Envelope(trkBounds);
				} else {
					bounds.expandToInclude(trkBounds);
				}
			}
		}
		return bounds;
