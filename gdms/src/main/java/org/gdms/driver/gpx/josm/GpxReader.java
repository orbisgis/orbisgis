...
/**
 * Read a gpx file. Bounds are not read, as we caluclate them. @see
...
public class GpxReader {
	...
	private enum State {
		init, metadata, wpt, rte, trk, ext, author, link, trkseg, copyright
	}
	...
	private class Parser extends DefaultHandler {
		...
		private Coordinate parseLatLon(Attributes atts) {
			return new Coordinate(parseCoord(atts.getValue("lon")),
					parseCoord(atts.getValue("lat")));
		}

		@Override
		public void startElement(String namespaceURI, String qName,
				String rqName, Attributes atts) throws SAXException {
			elements.push(qName);
			switch (currentState) {
			case init:
		...
		public void characters(char[] ch, int start, int length) {
			/**
			 * Remove illegal characters generated by the Nokia Sports Tracker
			 * device. Don't do this crude substitution for all files, since it
			 * would destroy certain unicode characters.
			 */
		...
		private Map<String, Object> getAttr() {
			switch (currentState) {
			case rte:
				return currentRoute.attr;
			case metadata:
				return currentData.attr;
			case wpt:
				return currentWayPoint.attr;
			case trk:
				return currentTrackAttr;
			default:
				return null;
			}
		}
		...
		public void endElement(String namespaceURI, String qName, String rqName) {
			...
			case metadata:
				if (qName.equals("name")) {
					currentData.attr.put(GpxData.META_NAME, accumulator
							.toString());
				} else if (qName.equals("desc")) {
					currentData.attr.put(GpxData.META_DESC, accumulator
							.toString());
				} else if (qName.equals("time")) {
					currentData.attr.put(GpxData.META_TIME, accumulator
							.toString());
				} else if (qName.equals("keywords")) {
					currentData.attr.put(GpxData.META_KEYWORDS, accumulator
							.toString());
			...
			case author:
				if (qName.equals("author")) {
					...
					currentData.attr.put(GpxData.META_AUTHOR_NAME, accumulator
							.toString());
				} else if (qName.equals("email")) {
			...
			case copyright:
				if (qName.equals("copyright")) {
					...
					currentData.attr.put(GpxData.META_COPYRIGHT_YEAR,
			...
			case link:
				...
				if (currentState == State.author) {
					...
					((Collection<GpxLink>) attr.get(GpxData.META_LINKS))
							.add(currentLink);
				}
				break;
			case wpt:
				if (qName.equals("ele") || qName.equals("magvar")
					...
					currentWayPoint.attr.put(qName, accumulator.toString());
												 ...
												 */) {
					try {
						currentWayPoint.attr.put(qName, Float
								.parseFloat(accumulator.toString()));
					} catch (Exception e) {
