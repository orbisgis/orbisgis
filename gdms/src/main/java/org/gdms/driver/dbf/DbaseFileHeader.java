...
public class DbaseFileHeader {
	...
	public int removeColumn(String inFieldName) {
		...
		for (int i = 0, j = 0; i < myFieldDescriptions.length; i++) {
			if (!inFieldName
					.equalsIgnoreCase(myFieldDescriptions[i].myFieldName.trim())) {
	...
	public void readHeader(BigByteBuffer2 in) throws IOException {
		...
		myNumFields = (myHeaderLength - FILE_DESCRIPTOR_SIZE - 1)
	 ...
	 * write the output file type. out.writeByte(myFileType); // write the date
	 * stuff Calendar c = Calendar.getInstance(); c.setTime(new Date());
	 * out.writeByte(c.get(Calendar.YEAR) - 1900);
	 * out.writeByte(c.get(Calendar.MONTH) + 1);
	 * out.writeByte(c.get(Calendar.DAY_OF_MONTH)); // write the number of
	 * records in the datafile. out.writeInt(myNumRecords); // write the length
	 * of the header structure. out.writeShort(myHeaderLength); // write the
	 * length of a record out.writeShort(myRecordLength); // write the reserved
	 * bytes in the header for (int i = 0; i < 20; i++) out.writeByte(0); //
	 * write all of the header records int tempOffset = 0; for (int i = 0; i <
	 * myFieldDescriptions.length; i++) { // write the field name for (int j =
	 * 0; j < 11; j++) { if (myFieldDescriptions[i].myFieldName.length() > j) {
	 * out.writeByte((int) myFieldDescriptions[i].myFieldName.charAt( j)); }
	 * else { out.writeByte(0); } } // write the field type
	 * out.writeByte(myFieldDescriptions[i].myFieldType); // write the field
	 * data address, offset from the start of the record.
	 * out.writeInt(tempOffset); tempOffset +=
	 * myFieldDescriptions[i].myFieldLength; // write the length of the field.
	 * out.writeByte(myFieldDescriptions[i].myFieldLength); // write the decimal
	 * count. out.writeByte(myFieldDescriptions[i].myDecimalCount); // write the
	 * reserved bytes. for (int j = 0; j < 14; j++) out.writeByte(0); } // write
	 * the end of the field definitions marker out.writeByte(0x0D); }
	 */
