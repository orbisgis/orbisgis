...
public class OrderByOperator extends AbstractExpressionOperator implements
		...
		try {
			...
			for (int i = 0; i < fieldIndexes.length; i++) {
				...
				if (field instanceof Field) {
					fieldIndexes[i] = getFieldIndexByName(source,
							((Field) field).getFieldName());

				} else if (field instanceof FunctionOperator) {
					Field[] fieldReferences = field.getFieldReferences();
					for (Field fieldF : fieldReferences) {
						fieldF.setFieldContext(fieldContext);
						fieldF.setFieldIndex(getIndex(source.getMetadata(),
								fieldF.getFieldName()));
					}
				}
			}
			...
			for (int i = 0; i < rowCount; i++) {
				...
				for (int field = 0; field < fields.size(); field++) {
					Expression expre = fields.get(field);
					if (expre instanceof Field) {
	...
	public void addCriterium(Expression field, boolean asc) {
		fields.add(field);
		orders.add(asc);
	}
