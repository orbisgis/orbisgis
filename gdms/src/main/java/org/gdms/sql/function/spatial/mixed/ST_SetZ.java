...
public class ST_SetZ implements Function {
	...
	public Value evaluate(DataSourceFactory dsf,Value[] args) throws FunctionException {
		...
		if (args[1] instanceof RasterValue) {
			try {
				if (null == dem) {
	...
	public String getDescription() {
		return "This function modify (or set) the z component of (each vertex of) the "
				+ "geometric parameter to the corresponding value given by a numeric value or a raster.";
	}

	public Arguments[] getFunctionArguments() {

		return new Arguments[] { new Arguments(Argument.GEOMETRY,
				Argument.NUMERIC), new Arguments(Argument.GEOMETRY,
				Argument.RASTER) };
	}
	...
	public String getSqlOrder() {
		return "ST_SetZ(the_geom, value [raster]);";
	}
