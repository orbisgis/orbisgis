...
public class RowTransactionSupport {
	...
	public RowTransactionSupport(DataSource ds, MetadataEditionSupport mes)
	...
	public long insertRowAt(long rowIndex, Value[] values)
			throws DriverException {
		long index = internalBuffer.insertRow(values);
	...
	/**
	 * @see org.gdms.data.edition.DataWare#cancel()
	 */
	 ...
	 * @param originalRow
	 *            row with the values of the row before the modification if it's
	 *            the first time the row is modified. If it isn't can be null.
	 * @return
	 * @throws DriverException
	 */
	public long setFieldValue(long row, int fieldId, Value value,
			Value[] originalRow) throws DriverException {
		// Get where's the pk
	...
	public Value getFieldValue(long rowIndex, int fieldId)
			throws DriverException {
		int realIndex = rowTable.getIndexLocation((int) rowIndex);
		int flag = rowTable.getIndexFile((int) rowIndex);

		if (flag == RowTable.EXPANSION_FILE) {
			return internalBuffer.getFieldValue(realIndex, fieldId);
		} else {
			/*
	...
	protected long getOriginalRowIndex(long rowId) {
		if (rowTable.getIndexFile((int) rowId) == RowTable.EDITED_SOURCE) {
			...
			throw new RuntimeException(rowId
					+ " th row does not exists in the original source");
		}
	}
