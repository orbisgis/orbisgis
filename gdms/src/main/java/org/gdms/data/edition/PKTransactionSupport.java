...
public class PKTransactionSupport extends RowTransactionSupport {
	...
	public void beginTrans(long rc) throws DriverException {
		...
		for (int i = 0; i < rc; i++) {
			...
			info.add(new NoEditionInfo(((PKInternalDataSource) ds)
					.getPKValue(i), i));
		}
	}
		...
		if (isOriginal) {
			...
			if (ei instanceof NoEditionInfo) {
				DeleteEditionInfo dei = new DeleteEditionInfo(
						((NoEditionInfo) ei).getPk(), getDS().getPKNames(),
						tableName, driver);
				deletedPKs.add(dei);
			} else if (ei instanceof UpdateEditionInfo) {
				DeleteEditionInfo dei = new DeleteEditionInfo(
						((UpdateEditionInfo) ei).getOriginalPK(), getDS()
								.getPKNames(), tableName, driver);
				deletedPKs.add(dei);
			}
		}
	...
	public long setFieldValue(long row, int fieldId, Value value)
		...
		if (original.get((int) row)) {
			...
			if (ei instanceof NoEditionInfo) {
				...
				UpdateEditionInfo uei = new UpdateEditionInfo(tableName,
						getDS().getPKNames(), getDS(), driver, (int) index, pk,
						internalBuffer);
