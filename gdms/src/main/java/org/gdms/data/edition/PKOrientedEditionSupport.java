...
public class PKOrientedEditionSupport {
	private static Logger logger = Logger
			.getLogger(PKOrientedEditionSupport.class.getName());
	...
	public void commitTrans() throws DriverException, FreeingResourcesException {
		if (driver instanceof DBReadWriteDriver) {
			try {
				((DBReadWriteDriver) driver).beginTrans(ds.getConnection());
		...
		try {
			...
			if (driver instanceof DBReadWriteDriver) {
				try {
					((DBReadWriteDriver) driver).rollBackTrans(ds
							.getConnection());
		...
		if (driver instanceof DBReadWriteDriver) {
			try {
				((DBReadWriteDriver) driver).commitTrans(ds.getConnection());
			} catch (SQLException e) {
				throw new DriverException(e);
			}
		}
