...
public class DBQuerySourceDefinition extends DBTableSourceDefinition {
	 ...
	 * 
	 * @param name
	 *            Name of the data source in this system
	 * @param sql
	 *            SQL instruction that defines the data source
	 * @param driverName
	...
	public DataSource createDataSource(String tableName, String tableAlias,
		...
		try {
			return getDataSourceByQuery(getDataSourceFactory(), tableName,
					tableAlias);
	...
	public void freeResources(String name)
		...
		if (viewName != null) {
			...
			try {
				DBTableDataSourceAdapter dbds = getDataSourceByQuery(
						getDataSourceFactory(), name, name);
	 ...
	 * 
	 * @throws SQLException
	 *             If cannot create the view in the dbms
	 * @throws DriverException
	 */
	private DBTableDataSourceAdapter getDataSourceByQuery(
			DataSourceFactory dsf, String tableName, String tableAlias)
			throws SQLException, DriverException {
		DBDriver driver = (DBDriver) dsf.getDriverManager().getDriver(
				driverName);
		((ReadOnlyDriver) driver).setDataSourceFactory(dsf);

		// Create the adapter
		DBTableDataSourceAdapter adapter = new DBTableDataSourceAdapter(
				tableName, tableAlias, def, driver);
	 ...
	 */
	private String createView(DataSourceFactory dsf,
			DBTableDataSourceAdapter dbds, String sql) throws SQLException {
		/*
