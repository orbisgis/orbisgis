...
public interface DataSource extends ReadAccess {
    ...
    /**
     * The edition events will be stored but not notified. When
     * the status changes a multipleModification event will be
     * sent to the listeners
     */
    public static final int STORE = 2;

    /**
     * Opens the DataSource to access the data it contains. If the data is accessed
     * without a previous opening a ClosedDataSourceException is thrown.
     *
     ...
     * retrieved. Any attempt to do so will result in a ClosedDataSourceException.
    ...
    /**
     * Commits the changes made during the transaction. The DataSource is closed after the commit
     * so any spatial or alphanumeric index will be cleared
     *
     * @throws DriverException If the transaction could not  be commited
     * @throws FreeingResourcesException The commit was correctly done but a problem occured
    ...
    /**
     * Saves the data in the parameter DataSource in the source of this DataSource. Both
     * DataSource's must have the same schema, the same metadata. This DataSource must
     * be closed before any call to this method
     *
     * @param ds DataSource with the data
     *
     * @throws DriverException if the operation fails
    ...
    /**
     * Defines the behaviour of the DataSource when an edition event happens. It can
     * be set to DISPATCH, STORE, IGNORE. It's set to DISPATCH when the DataSource opens
     *
     ...
     * @param driverType driver specific type name
     *
     * @throws DriverException
     */
     ...
     * @throws DriverException
     */
    void addField(String name, String driverType, String[] paramNames, String[] paramValues) throws DriverException;
     ...
     * if the field contains a valid value and returns a String with a message to the user
     * if it is not
     *
     * @param fieldId
     * @param value
     ...
     * it doesn't check the source and only checks wheter the source has been modified
