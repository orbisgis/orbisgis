...
public class InnerDBUtils {
    /**
     * Builds a WHERE clause with the and operation of the equality between
     * each field name and field value
     *
     * @param pks String array with the field values
     * @param fieldNames String array with the field names
     *
     * @return String
     */
    ...
    /**
     * Creates a delete statement in the specified table in the row specified
     * by the field names and field values
     *
     * @param pks values to specify the row to delete
     * @param names names of the fields
     * @param tableName name of the table
     * @param vWriter DOCUMENT ME!
     *
     * @return String
     */
    public static String createDeleteStatement(Value[] pks, String[] names,
        ...
        StringBuffer sql = new StringBuffer("DELETE FROM ").append(tableName)
                                                           .append(" WHERE ")
                                                           .append(names[0])
                                                           .append("=").append(pks[0].getStringValue(
                    vWriter));

        for (int i = 1; i < pks.length; i++) {
            sql.append(" AND ").append(names[i]).append("=").append(pks[i].getStringValue(
                    vWriter));
        }
     ...
     * @param names names of the fields
     * @param types types of the fields. Must have the same length than names
     *
     ...
     *
     * @param tableName table where the index will be created
     * @param pkNames name of the fields where the index will be created
     *
     * @return String
     */
     ...
     *
     * @param type java.sql.Types constant
     *
     * @return String
     *
     ...
     * @param row values to be inserted
     * @param fieldNames names of the fields to be inserted
     * @param vWriter DOCUMENT ME!
     *
     * @return String
     */
    public static String createInsertStatement(String tableName, Value[] row,
        ...
        sql.append("INSERT INTO ").append(tableName).append("(").append(fieldNames[0]);
     ...
     *
     * @param tableName table name
     * @param pk values of the primary key
     * @param pkNames name of the primary key fields
     * @param fieldNames name of all fields
    ...
    public static String createUpdateStatement(String tableName, Value[] pk,
        ...
        sql.append("UPDATE ").append(tableName).append(" SET ")
           .append(fieldNames[0]).append(" = ").append(values[0].getStringValue(
                vWriter));

        for (int i = 1; i < fieldNames.length; i++) {
            sql.append(", ").append(fieldNames[i]).append(" = ").append(values[i].getStringValue(
                    vWriter));
        }

        sql.append(" WHERE ").append(pkNames[0]).append(" = ").append(pk[0].getStringValue(
                vWriter));

        for (int i = 1; i < pkNames.length; i++) {
            sql.append(" AND ").append(pkNames[0]).append(" = ").append(pk[0].getStringValue(
                    vWriter));
        }
