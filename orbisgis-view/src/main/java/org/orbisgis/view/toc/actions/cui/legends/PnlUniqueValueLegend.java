...
import org.orbisgis.view.toc.actions.cui.legends.table.UniqueValueLegendTableModel;

@Deprecated
public class PnlUniqueValueLegend extends PnlAbstractClassifiedLegend {
    private static Logger UILOGGER = Logger.getLogger("gui."+PnlUniqueValueLegend.class);
	...
	protected void addOneAction() {
		...
		if (rowCount < 32) {
			try {
				...
				UILOGGER.error("Cannot access layer data", e);
			}
		} else {
			UILOGGER.error(
	...
	private void syncFieldsWithLegend() {
		...
		try {
			...
			UILOGGER.error("Cannot populate field names", e);
		}
