...
import org.orbisgis.view.toc.actions.cui.legends.table.IntervalLegendTableModel;

@Deprecated
public class PnlIntervalLegend extends PnlAbstractClassifiedLegend {

    private static Logger UILOGGER = Logger.getLogger("gui."+PnlIntervalLegend.class);
	...
	private void syncFieldsWithLegend() {
		...
		try {
			...
			UILOGGER.error("Cannot access layer fields", e);
		}
	...
	protected void addAllAction() {
		...
		try {
			...
			UILOGGER.error("Cannot calculate intervals", ex);
			return;
		} catch (DriverException e) {
			UILOGGER.error("Cannot calculate intervals", e);
			return;
		}
