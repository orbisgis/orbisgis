...
import org.gdms.triangulation.jts.JtsGeometryUtil;
import org.gdms.triangulation.jts.LineSegment3D;
...
public class TriangulatedIrregularNetwork {
  ...
  private static final Logger LOG = Logger.getLogger(TriangulatedIrregularNetwork.class);
  ...
  public TriangulatedIrregularNetwork(GeometryFactory geometryFactory,
    ...
    this.geometryFactory = geometryFactory;
    this.precisionModel = geometryFactory.getPrecisionModel();
    Coordinate c1 = new Coordinate(envelope.getMinX(), envelope.getMinY(), 0);
    Coordinate c2 = new Coordinate(envelope.getMaxX(), envelope.getMinY(), 0);
    Coordinate c3 = new Coordinate(envelope.getMaxX(), envelope.getMaxY(), 0);
    Coordinate c4 = new Coordinate(envelope.getMinX(), envelope.getMaxY(), 0);
  ...
  public void insertNode(final Coordinate coordinate) {
    List<Triangle> triangles = circumCircleIndex.query(new Envelope(coordinate));
  ...
  public void insertEdge(final LineString breakline) {
    ...
    for (int i = 1; i < coordinates.size(); i++) {
      Coordinate coordinate = coordinates.getCoordinate(i);
      LineSegment3D segment = new LineSegment3D(previousCoordinate, coordinate);
      insertEdge(segment);
  ...
  private void addBreaklineIntersect(final Triangle triangle,
    ...
    if (startCornerDistance < 0.01) {
      ...
      if (endCornerDistance < 0.01) {
        ...
        Triangle newTriangle = Triangle.createClockwiseTriangle(lc0, lc1,
  ...
  private void addTriangleTouchingOneCorner(Triangle triangle,
    Coordinate[] coords, Coordinate lc0, Coordinate lc1, int startCorner,
    int endEdge, double endEdgeDistance) {
  ...
  private void addTriangleStartCornerEndInside(Triangle triangle,
    Coordinate[] coords, int cornerIndex, Coordinate cCorner, Coordinate cInside) {
  ...
  private void addTrangleCornerAndEdgeTouch(Triangle triangle,
    Coordinate cPrevious, Coordinate c, Coordinate cNext, Coordinate cOpposite) {
    replaceTriangle(triangle, new Triangle[] {
  ...
  private void addBreaklineIntersect(final Triangle triangle,
    ...
    for (int i = 1; i < triCoords.length; i++) {
      ...
      if (!triCorner.equals2D(intersectCoord)
        ...
        double distance = new LineSegment(previousCoord, triCorner).distance(intersectCoord);
        if (distance == 0) {
  ...
  public double getElevation(Coordinate coordinate) {
    ...
    for (Triangle triangle : triangles) {
      ...
      LineSegment segment = JtsGeometryUtil.addLength(new LineSegment(
        ...
        + t0.distance(t2));
      Coordinate intersectCoordinate = oppositeEdge.intersection3D(segment);
